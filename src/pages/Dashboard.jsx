...
export default function Dashboard() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      setDashboardData(prev => ({
        ...
        revenueThisMonth:
          res.data.revenueThisMonth ?? res.data.incomeThisMonth ?? 0,
        expenseThisMonth:
          res.data.expenseThisMonth ?? res.data.totalExpenseThisMonth ?? 0,
  ...
  const { months, revenue, expense } = getMonthlyStats(dashboardData.transactions);
  ...
  const compteCourant = dashboardData.comptes.find(c => c.type === "courant");
    ...
    <div className="space-y-6 p-4 sm:p-6 ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 dashboard-container">
        ...
        <div
          ...
          className={`rounded-xl p-10 shadow-lg card card-2 cursor-pointer transition-all duration-300 ${activeCard === "revenu" ? "bg-[#6b5a49] text-white" : "bg-white dark:bg-[#2a2a2a] dark:text-[#f1e8dc] text-[#6b5a49]"}`}
        >
        ...
        <div
          ...
          className={`rounded-xl card card-3 p-10 shadow-lg cursor-pointer transition-all duration-300 ${activeCard === "depense" ? "bg-[#6b5a49] text-white" : "bg-white dark:bg-[#2a2a2a] dark:text-[#f1e8dc] text-[#6b5a49]"}`}
        >
