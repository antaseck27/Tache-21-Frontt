      ...
      <main className="ml-0 md:ml-[50%] flex-1 flex items-center justify-center p-6 bg-[#fdf8f2]">
        <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-5 space-y-3">
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            ...
            <button type="submit" disabled={loading} className="w-full py-2 rounded-lg bg-[#6b5a49] text-white font-medium text-sm hover:bg-[#5c4d3e] disabled:opacity-60">
              {loading ? "Inscription..." : "Créer mon compte"}
            </button>
          </form>

          <p className="text-center text-sm text-[#8f7e6b] mt-2">
            Déjà un compte ? <Link to="/login" className="text-[#bfa98a] font-medium hover:underline">Se connecter</Link>
          </p>
        </div>
      </main>
    </div>
